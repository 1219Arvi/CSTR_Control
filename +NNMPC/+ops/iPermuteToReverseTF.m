...
if isdlarray(fwdTFInput) && rank >= 2
    ...
    elseif isInternal && isempty(inputLabels)
        ...
        if rank < numel(inputLabels) && strcmp(inputLabels, 'CBT')
            % Drop the least significant labels
            fwdTFInput = permute(stripdims(fwdTFInput), permuteVec);
            revTFInput = permute(stripdims(fwdTFInput), rank:-1:1);
        else
